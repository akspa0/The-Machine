...
def separate_single_audio_file(input_file: Path, output_root: Path, model_path: str) -> Dict:
    """
    Dedicated separation for single-file (non-call) inputs.
    Outputs vocals.wav and instrumental.wav in a dedicated folder.
    """
    call_id = input_file.stem
    channel = 'mono'
    call_dir = output_root / call_id
    call_dir.mkdir(exist_ok=True)
    temp_out = call_dir / '_tmp_single'
...
def separate_audio_file(input_file: Path, output_root: Path, model_path: str) -> Dict:
    ...
    if call_id and channel:
        ...
        # Dedicated single-file separation path
        return separate_single_audio_file(input_file, output_root, model_path)
